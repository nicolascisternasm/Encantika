...
export default async function StoreLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: config } = await supabase
    ...
    .select('nombre_tienda, numero_whatsapp, url_instagram, url_mercadolibre, tema, fuente_titulos, layout')
    .single()

  const temaConfig = getTema(config?.tema)
  const fuenteConfig = getFuente(config?.fuente_titulos)
  const layout = (config as { layout?: string } | null)?.layout ?? 'clasico'
    ...
    <footer className="bg-onyx text-white">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 pt-16 pb-12">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 sm:gap-8">
          <div>
            <Image src="/logo.png" alt="Encantika" width={140} height={36} className="h-9 w-auto"
              style={{ filter: 'brightness(0) invert(1)' }} />
            <p className="mt-4 text-sm text-encantika-stone tracking-wide">Brilla con magia</p>
          </div>
          <div>
            <p className="text-[11px] uppercase tracking-[.15em] text-encantika-stone mb-5">Navegación</p>
          ...
          <div>
            <p className="text-[11px] uppercase tracking-[.15em] text-encantika-stone mb-5">Síguenos</p>
            <ul className="space-y-3">
              {config?.url_instagram && (
                <li>
                  <a href={config.url_instagram} target="_blank" rel="noopener noreferrer"
              ...
              {config?.numero_whatsapp && (
                <li>
                  <a href={`https://wa.me/${config.numero_whatsapp}`} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-2 text-sm text-white/70 hover:text-white transition-colors">
              ...
              {config?.url_mercadolibre && (
                <li>
                  <a href={config.url_mercadolibre} target="_blank" rel="noopener noreferrer"
      ...
      <div className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 py-4 flex items-center justify-between">
          <p className="text-[11px] text-encantika-stone">© 2026 Encantika · Todos los derechos reservados</p>
          <Link href="/administracion" className="text-[11px] text-encantika-stone hover:text-white transition-colors">
            Administración
          </Link>
        </div>
      </div>
  ...
  const whatsapp = <WhatsAppButton numero={config?.numero_whatsapp ?? null} />

  // ── Layout lateral: sidebar izquierdo + contenido derecha ───────────────────
  if (isLateral) {
    return (
  ...
  // ── Resto de layouts: header arriba + contenido ─────────────────────────────
  return (
