...
import CaracteristicasEditor from '@/components/admin/CaracteristicasEditor'
...
export default async function EditarProductoPage({
  ...
  // Stock: suma de variantes activas (incluye variante std si existe)
  ...
  // Insumos para productos fabricados
  ...
  const caracteristicasIniciales = ((producto as any).caracteristicas as any[] ?? [])
    .map((c: any) => ({ nombre: String(c.nombre ?? ''), valor: String(c.valor ?? '') }))
    .filter((c: any) => c.nombre || c.valor)
  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      <CollapsibleSection title="Características del producto" defaultOpen>
        <CaracteristicasEditor
          productoId={id}
          inicial={caracteristicasIniciales}
        />
      </CollapsibleSection>
